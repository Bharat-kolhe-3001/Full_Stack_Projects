// PoultrySmart — Mock Data Layer (React Version)

export const users = [
  { id: 'u1', name: 'Admin', email: 'admin@poultry.com', phone: '9000000000', role: 'admin', avatar: null, verified: true },
...
export const farms = [
  { id: 'f1', ownerId: 'u2', name: "Ravi's Green Farm", location: 'Nashik, Maharashtra', state: 'Maharashtra', district: 'Nashik', totalBirds: 5000, activeBatches: 3, status: 'verified', rating: 4.8, established: 2018, certifications: ['Organic Certified', 'FSSAI'] },
  { id: 'f2', ownerId: 'u3', name: "Sunita Poultry Hub", location: 'Amravati, Maharashtra', state: 'Maharashtra', district: 'Amravati', totalBirds: 3200, activeBatches: 2, status: 'verified', rating: 4.6, established: 2020, certifications: ['FSSAI'] },
  { id: 'f3', ownerId: 'u4', name: "Ali Agro Farm", location: 'Aurangabad, Maharashtra', state: 'Maharashtra', district: 'Aurangabad', totalBirds: 8000, activeBatches: 4, status: 'pending', rating: 4.3, established: 2019, certifications: [] },
];
...
// ── Auth ──
export const credMap = {
  'admin@poultry.com': { pass: 'admin123', userId: 'u1' },
...
export const stats = {
  todayEggs() {
    ...
    return productionLogs.filter(l => l.date === today && l.farmId === 'f1').reduce((s, l) => s + l.netEggs, 0) || 4280;
  },
  monthRevenue() {
    const mo = new Date().toISOString().substr(0, 7);
    return transactions.filter(t => t.farmId === 'f1' && t.type === 'income' && t.date.startsWith(mo)).reduce((s, t) => s + t.amount, 0) || 91400;
  },
  pendingOrders() { return orders.filter(o => o.status === 'pending').length; },
  totalBirds() { return flocks.filter(f => f.farmId === 'f1' && f.status === 'active').reduce((s, f) => s + f.currentCount, 0); },
  platformEggsTracked: 48200000,
  platformFarms: 12400,
  platformOrders: 3100000,
};

// ── i18n ──
export const i18nStrings = {
  en: { welcome: 'Good morning', todayEggs: "Today's Eggs", liveBirds: 'Live Birds', pendingOrders: 'Pending Orders', monthRevenue: 'Monthly Revenue', farmName: "Ravi's Green Farm" },
  hi: { welcome: 'सुप्रभात', todayEggs: 'आज के अंडे', liveBirds: 'जीवित पक्षी', pendingOrders: 'लंबित ऑर्डर', monthRevenue: 'मासिक राजस्व', farmName: 'रवि का हरित फार्म' },
  mr: { welcome: 'शुभ सकाळ', todayEggs: 'आजची अंडी', liveBirds: 'जिवंत पक्षी', pendingOrders: 'प्रलंबित ऑर्डर', monthRevenue: 'मासिक उत्पन्न', farmName: 'रवीचे हरित फार्म' },
};
